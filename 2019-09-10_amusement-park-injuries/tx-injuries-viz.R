...
tx_df <- tx_injuries %>%
  ...
  mutate(
    ...
    gender = ifelse(gender == "F", "Females", "Males")
  )
...
p1 <- ggplot(tx_df1, aes(x = month, y = n,
  ...
  labs (
    title = "Injuries in amusement parks in Texas",
    subtitle = "{closest_state}",
    caption = "#TidyTuesday, 2019-09-10 - @jmcastagnetto / Jesus M. Castagnetto",
    x = "",
    y = ""
  ) +
  facet_wrap(~gender) +
...
p2 <- ggplot(tx_df2, aes(y = year, x = age, fill = ..x..)) +
  ...
  labs(
    ...
    subtitle = "Young males seem to be more injury prone\n(Median value shown as a vertical line)",
    caption = "#TidyTuesday, 2019-09-10 - @jmcastagnetto / Jesus M. Castagnetto",
    y = "",
    x = "",
    fill = "Age (yrs)"
