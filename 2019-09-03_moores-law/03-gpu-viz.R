...
# remove rows w/o date_of_introduction
gpu <- gpu %>%
  filter(!is.na(date_of_introduction))

# --- main plot --

p1 <- ggplot(gpu, aes(x = decade, y = transistor_count)) +
  geom_pirate(aes(color = decade, fill = decade),
              bars_params = list(width = 0.1)) +
  scale_y_log10() +
  annotation_logticks(sides = "l", color = "white") +
  labs(
    title = "GPU transistor count distribution over the decades",
    subtitle = "Moore's Law dataset / #TidyTuesday 2019-09-03 ",
    y = "Transistor count",
    x = "Decade when the GPU was introduced",
...
ggsave(
  filename = here::here("2019-09-03_moores-law/gpu-decades-plot.png"),
  plot = p12,
  width = 14,
  height = 9,
  device = agg_png,
